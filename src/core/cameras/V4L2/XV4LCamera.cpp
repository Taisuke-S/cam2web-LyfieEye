...
static void show_videoCapability(int fd)
{
...
//			printf("#%d %c%c%c%c format_width %d,  format_height %d\n",i,
//						frame_size.pixel_format >>0,frame_size.pixel_format >>8,frame_size.pixel_format >>16, frame_size.pixel_format >>24,
...
//										NOTIMPLEMENTED();
			...
			while (ioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &frame_interval) == 0) {
				...
				printf("#%d %c%c%c%c %d x %d@%d\n",cnt,
						frame_size.pixel_format >>0,frame_size.pixel_format >>8,frame_size.pixel_format >>16, frame_size.pixel_format >>24,
						frame_size.discrete.width,frame_size.discrete.height,frame_rate);	
				++cnt;
...
bool XV4LCameraData::Init( )
{
    ...
    {
		// Added for LyfieEye200 to show all videoCapability information
		show_videoCapability(VideoFd);
